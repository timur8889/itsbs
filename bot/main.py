        ...
        try:
            if os.path.exists('admins_backup.json'):
                with open('admins_backup.json', 'r', encoding='utf-8') as f:
                    ...
                    # Обновляем только существующие отделы
                    for dept, admins in loaded_admins.items():
                        if dept in cls.ADMIN_CHAT_IDS:
                            cls.ADMIN_CHAT_IDS[dept] = admins
        except Exception as e:
...
async def search_requests_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
...
# ==================== МАССОВАЯ РАССЫЛКА И УПРАВЛЕНИЕ АДМИНАМИ ====================
...
def main() -> None:
    ...
    try:
        ...
        print("   • 💾 Ручные бэкапы")
        print("   • 🚨 Уведомления о просроченных заявках")
